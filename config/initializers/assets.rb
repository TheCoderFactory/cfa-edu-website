# Be sure to restart your server when you modify this file.

# Version of your assets, change this if you want to expire all your assets.
Rails.application.config.assets.version = '1.0'

# Add additional assets to the asset load path
# Rails.application.config.assets.paths << Emoji.images_path

# Precompile additional assets.
# application.js, application.css, and all non-JS/CSS in app/assets folder are already added.
# Rails.application.config.assets.precompile += %w( search.js )

Rails.application.config.assets.precompile += %w( booking.js )
Rails.application.config.assets.precompile += %w( dataTables.js )
Rails.application.config.assets.precompile += %w( blog_sidebar.js )
Rails.application.config.assets.precompile += %w( bootstrap-datetimepicker.js )
Rails.application.config.assets.precompile += %w( pickers )
Rails.application.config.assets.precompile += %w( close-modal )
Rails.application.config.assets.precompile += %w( main )
Rails.application.config.assets.precompile += %w( modernizr )
Rails.application.config.assets.precompile += %w( datetime-moment.js )
Rails.application.config.assets.precompile += %w( initialise-selects.js )
Rails.application.config.assets.precompile += %w( ckeditor/* )
